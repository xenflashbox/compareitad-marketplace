/**
 * Transaction process graph for ITAD RFP completion, vendor-pays pattern:
 *   - itad-rfp-completion-vendor-pays
 *
 * The winning vendor pays the seller (asset owner) for the assets.
 * The seller (customer role) drives the same state machine with a
 * $0.01 nominal checkout; the real vendor-to-seller payment moves
 * over Compare ITAD's escrow rails outside Sharetribe, triggered by
 * the platform backend when the transaction reaches acceptance.
 */
...
export const graph = {
  ...
  id: 'itad-rfp-completion-vendor-pays/release-1',
